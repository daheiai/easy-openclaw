...
- 若 `local-searxng`：确认本地地址（默认 `http://127.0.0.1:8081`）
...
- `local-searxng` 安装后必须验证 JSON 接口可用：`/search?...&format=json` 应返回 200 且可被 `jq` 解析
- 若出现“HTML 200 但 JSON 403”，视为 `search.formats` 未启用 `json`，必须先修复再宣告可用
...
依赖映射（最低要求）：
- 若第 4 项联网搜索选 `local-searxng`：`docker`、`/var/run/docker.sock` 可访问、`curl`、`jq`
...
- Docker socket 权限不足：先提示将当前用户加入 `docker` 组并重新登录会话；未完成前不得承诺“已可用”
...
- 联网搜索（若第 4 项开启）
  ...
  - 在 `~/.openclaw/workspace/TOOLS.md` 写入/更新“搜索服务”说明：
    - 默认 `local-searxng`：`http://127.0.0.1:8081/search?q=关键词&format=json`
    - 若用户选 `public-searx`：替换为用户提供的实例地址
    - 明确流程：先 SearXNG JSON 检索，再整理答案；必要时用 `browser`（profile=`openclaw`）snapshot
  - 若本地部署 SearXNG：优先使用 `-p 8081:8080`，避免与已有 `8080` 服务冲突
  - 若检测到“HTML 200 但 JSON 403”：引导修复 `search.formats`，确保至少包含 `html` 与 `json`
...
- 对已选搜索源执行一次连通性测试：
  - 本地：`curl -s "http://127.0.0.1:8081/search?q=openclaw&format=json" | jq '.results | length'`
  - 公共实例：将地址替换为用户提供实例并执行同样测试
- 若本地出现“HTML 200 但 JSON 403”，必须执行以下修复后复测：
