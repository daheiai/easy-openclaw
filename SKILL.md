...
- 第 5 项为“权限模式”三档：强烈建议保持 `coding`；`minimal` 为纯聊天机器人；`full` 为完全放开；`coding/full` 下可选叠加 exec 高危操作审批（allowlist + ask=on-miss）并通过 `approvals.exec` 配置审批提示投递
...
> 5. 权限模式（强烈建议别改）：默认维持现状 `coding`（正常情况下权限已足够）；若想把它当“纯聊天机器人/绝对安全”，才选 `minimal`；若想完全放开才选 `full`。另外可选为 `coding/full` 叠加“高危操作审批”（不在 allowlist 的命令会弹出批准）。
>
> 请按格式回复：`渠道 discord; 1 开, 2 记忆增强, 3 开, 4 开, 5 维持现状`
> 第 5 项可选值：`维持现状` / `维持现状+审批` / `完全开放` / `完全开放+审批` / `最小安全`
...
> 7. Discord 审批按钮（可选）：在 Discord 内用按钮审批（仅当第 5 项选择带 `+审批` 时才有意义）
>
> 若当前渠道为 `feishu`：
> 8. 飞书限额优化：探测逻辑加 24h 缓存，避免每分钟探测把月限额跑满
>
> 若当前渠道为 `telegram`：
> - 本轮无必选项（若你在第 1 轮已选择带 `+审批`，审批提示投递走第 1 层权限模式的统一配置），将直接进入第 3 轮
>
> 请按格式回复：`discord 用 6 开, 7 关；feishu 用 8 开`（只填出现的编号）
...
### 若第 5 项开启（权限模式）
询问：
- 你想要哪种模式：`维持现状（默认，coding）` / `完全开放（full）` / `最小安全（minimal）`（也可选择带 `+审批` 的两种）
...
- 只有在用户选择带 `+审批`（`维持现状+审批` / `完全开放+审批`）时，才继续写入 `approvals.exec.*` 与 `exec-approvals.json` 默认策略
...
- 权限模式（若第 5 项开启）
  - 若用户选择“维持现状”或“维持现状+审批”：默认保持 `tools.profile: "coding"`
  - 若探测到当前权限过低：提示用户先执行 `openclaw config set tools.profile coding`
  - 若用户选择“完全开放”或“完全开放+审批”：写入 `tools.profile: "full"` 且 `agents.defaults.sandbox.mode: "off"`
...
- Discord 审批按钮（若第 7 项开启）
  - 前提：第 5 项已选择带 `+审批`，且审批提示已投递到 Discord（`approvals.exec.mode=session/targets/both`）
...
### 3.1 同步 exec-approvals 默认策略（若第 5 项选择带 `+审批`）
...
- 若用户选择带 `+审批`：`approvals defaults` 中 `ask=on-miss`、`askFallback=deny`、`security=allowlist`
