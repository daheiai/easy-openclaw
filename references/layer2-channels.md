...
- 若用户开启这一项，执行阶段还必须同步写入 `tools.exec.host="gateway"`、`tools.exec.security="allowlist"` 与 `tools.exec.ask="on-miss"`
- 目标效果必须是“常见命令别打扰用户，高风险命令再审批”；若出现 `cat` / `grep` / `ls` 这类命令也频繁弹审批，说明配置写错了
