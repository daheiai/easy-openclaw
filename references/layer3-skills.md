...
- 安装流程不得一刀切；必须按条目类型执行：
  ...
  - 内置能力型条目：直接按本文件给出的固定方案启用/验证，不要伪装成“去安装某个仓库 Skill”。
  - 任务型条目（如大黑AI日报）：直接创建 / 修复对应 cron 与投递配置，再做手动触发验证；不要先假装它是一个独立 Skill 仓库。
...
- 条目性质：这不是一个独立仓库 Skill，也不是 ClawHub 安装项；它是当前会话内直接创建的日报 cron 任务。
- RSS 读取规则（强制）：
  - 只读取最新一个 `item`
  - 标题优先使用 `title`
  - 时间优先使用 `pubDate`
